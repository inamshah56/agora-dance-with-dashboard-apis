...
export async function forgotPassword(req, res) {
	try {
		...
		expiry.setSeconds(expiry.getSeconds() + 5 * 60);
...
export async function verifyOtp(req, res) {
	try {
		...
		if (user.otp !== parseInt(otp, 10)) {
			...
			return user.otp_count === 2 ? validationError(res, 'Maximum OTP attempts reached. Please regenerate OTP.') : validationError(res, 'Invalid OTP');
		}
...
export async function setNewPassword(req, res) {
	try {
		...
		if (user.can_change_password === false) {
			return UnauthorizedError(res, "OTP not verified. Please verify OTP first.");
		}
